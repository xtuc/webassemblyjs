...
function decodeControlCharacter(char: string) {
  switch (char) {
    case "t":
      return 0x09;
    case "n":
...
const ESCAPE_CHAR = 92; // backslash
const QUOTE_CHAR = 34; // backslash

// parse string as per the spec:
// https://webassembly.github.io/spec/core/multipage/text/values.html#text-string
export function parseString(value: string): Array<Byte> {
  const byteArray = [];
  let index = 0;
  ...
  while (index < value.length) {
    ...
    if (charCode === ESCAPE_CHAR) {
      const firstChar = value.substr(index + 1, 1);
      const decodedControlChar = decodeControlCharacter(firstChar);
      if (decodedControlChar !== -1) {
        ...
        const hexValue = value.substr(index + 1, 2);
